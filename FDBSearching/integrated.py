"""
FDB Searching tool - Integrated mode for use with central connection manager.

This module provides FDB searching functionality that integrates with the
...
def get_fdb_info(ssh_client, switch_ip, fdb_cache, force_refresh=False):
    ...
    if switch_ip in fdb_cache and not force_refresh:
        ...
        if time_since_cache < cache_duration:
            ...
            user_input = (
                input(
                    f"\nFDB cache for {switch_ip} is {minutes_ago} minutes old. Refresh from switch (y/n)? "
                )
                .lower()
            )
            if user_input != "y":
    ...
    # Cache the data
    fdb_cache[switch_ip] = {"data": fdb_output, "timestamp": current_time}
...
def find_camera_by_mac(mac_address, cameras):
    ...
    for camera in cameras:
        normalized_csv_mac = (
            camera["mac_address"].replace(":", "").replace("-", "").lower()
        )
...
def check_mac_database(user_mac, database_file="macdatabase.txt"):
    """Check if MAC address exists in database."""
    # Remove colons if present and take the first 6 characters (first three octets)
    ...
    try:
        ...
        print(f"Warning: {database_file} not found")
        return True  # Allow anyway if database file missing

    return False  # Return False if no match is found


def mode1_mac_search(conn_manager, cameras, fdb_cache):
    ...
    while True:
        ...
        while True:
            ...
            try:
                ...
                ssh = conn_manager.get_connection(full_ip)
                ...
                if matching_line:
                    ...
                    stdin, stdout, stderr = ssh.exec_command(
                        f"show ports {port_number} information"
                    )
                    ...
                    stdin, stdout, stderr = ssh.exec_command("show lldp neighbors")
                    ...
                    stdin, stdout, stderr = ssh.exec_command(
                        f"show ports {port_number} description"
                    )
                ...
                user_choice = input(
                    "Do you want to try another IP for this MAC address (y/n)? "
                ).lower()
                if user_choice != "y":
        ...
        continue_program = input(
            "Do you want to check another MAC address (y/n)? "
        ).lower()
        if continue_program != "y":
            break  # Return to main menu


def mode2_port_search(conn_manager, cameras, fdb_cache):
    ...
    while True:
        ...
        try:
            # Get connection from manager
            ssh = conn_manager.get_connection(switch_ip)
            ...
            for line in fdb_output.splitlines():
                ...
                if port_number in line:
                    # Extract MAC address from the line (usually first column)
                    parts = line.split()
                    if len(parts) > 0:
                        ...
                        if (
                            ":" in mac_candidate
                            or "-" in mac_candidate
                            or len(mac_candidate.replace(" ", "")) == 12
                        ):
            ...
            if port_macs:
                ...
                for mac in port_macs:
                    ...
                    if camera:
                        ...
                        print(
                            f"    MAC Address (Column E): {camera['mac_address']} (verified)"
                        )
                    else:
                        print(f"\n  MAC: {mac}")
...
def run_interactive(conn_manager):
    """
    Run FDB Searching in interactive mode with central connection manager.

    Args:
        conn_manager: Central ConnectionManager instance
    """
    print("\n" + "=" * 60)
    ...
    while True:
        ...
        if choice == "1":
            mode1_mac_search(conn_manager, cameras, fdb_cache)
        elif choice == "2":
            mode2_port_search(conn_manager, cameras, fdb_cache)
        elif choice == "3":
